    ...
    render() {
        if (this.state.fastestLappers.length > 0) {
            if (this.state.type !== "stackedColumn") {
                ...
                var data = this.state.fastestLappers.map(x => ({ type: "stackedColumn", showInLegend: true, name: x.name, dataPoints: x.fastestLapsByYear.map(yearFastestLap => ({ label: yearFastestLap.year, x: yearFastestLap.year, y: yearFastestLap.fastestLapCount })) }));
